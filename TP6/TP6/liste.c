...
int DeleteLinkedListElem(LinkedList* list, SingleLinkedListElem* item) {
	...
	SingleLinkedListElem* prec = NULL;

	// Si 1 seul élément dans la liste
	...
	// Recherche du maillon dans le reste de la liste
	...
	// Si il est en queue on supprime

	if ((prec != NULL) && (tmp == item) && (tmp->next == NULL))
	{
		prec->next = item->next;
		list->size--;
		free(item);
